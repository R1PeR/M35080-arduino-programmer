...
#define NEW_VIN       "XX12345" // Put here reqired lst 7 characters of your VIN (must be 2 letters+5 digits)
#define NEW_MILAGE_KM 1234567 // Put here required mileage in KILOMETERS
...
//  write_odometer(NEW_MILAGE_KM);
  ...
  print_status(status);
  if(status & STATUS_BP0 || status & STATUS_BP1)
  {
    if (write_enable())
    {
      SPI.beginTransaction(spiSettings);
  ...
  // if (find_vin()) {
  //   Serial.println("Vin found, changing vin..");
  //   write_vin(NEW_VIN);
  //   read_buf(0x000, eeprom, sizeof(eeprom));
  // }
  ...
  Serial.println("M35080 Programmer v2");
  Serial.println("1. Read Vin");
  Serial.println("2. Write Vin");
  Serial.println("3. Read Mileage");
  Serial.println("4. Write Mileage");
...
void loop() {
  ...
  switch(menuOption)
  {
    ...
    case 2:
      {
        ...
        if(find_vin())
        {
          Serial.println("Existing vin is:")
          print_vin();
          Serial.println("Put vin to insert:")
          while(Serial.available() == 0);
          String newVin = Serial.readString();
          Serial.print("Change vin to:")
          Serial.print(newVin);
          Serial.println("?");
          Serial.println("1 - Yes 0 - No");
          while(Serial.available() == 0);
          int write = Serial.parseInt();
          if(write)
          {
            write_vin(newVin);
            read_buf(0x000, eeprom, sizeof(eeprom));
    ...
    case 4:
      {
        read_buf(0x000, eeprom, sizeof(eeprom));
        Serial.println("Existing mileage is is:")
        print_odometer();
        Serial.println("Put mileage to insert:")
        String newVin = Serial.readString();
        Serial.print("Change mileage to:")
        Serial.print(newVin);
        Serial.println("?");
        Serial.println("1 - Yes 0 - No");
        while(Serial.available() == 0);
        ...
        if(write)
        {
          write_odometer(NEW_MILAGE_KM);
          read_buf(0x000, eeprom, sizeof(eeprom));
          Serial.println("Reading mileage");
          print_odometer();
        }
      }
    break;
    ...
    break;
    default:
      Serial.println("Wrong number selected!")
      Serial.println("M35080 Programmer v2");
      Serial.println("1. Read Vin");
      Serial.println("2. Write Vin");
      Serial.println("3. Read Mileage");
